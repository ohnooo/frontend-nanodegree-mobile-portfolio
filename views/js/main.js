...
function updatePositions() {
  ...
  // Set array of phases to reduce layout calls
  var phases = [];
  for(var i=0; i<5; i++){
    phases[i] = Math.sin(currentSinVar + (i % 5));
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Check window height and render the appropriate number of pizzas
  if(windowHeight <= MOBILE){
    numberPizzas = 15;
  }else if(windowHeight > MOBILE && windowHeight <= TAB){
    numberPizzas = 25;
  }else if (windowHeight > TAB){
    numberPizzas = 35;
  }
